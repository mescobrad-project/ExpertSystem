...
def test_get_run(db: Session) -> None:
    ...
    stored = RunController.read(
        db=db, resource_id=run["obj"].id, criteria={"deleted_at": None}
    )
    assert stored
...
def test_update_run(db: Session) -> None:
    run1 = seed_run(db)
    run2 = update_seed_run(db, run1["obj"].id)
...
def test_delete_run(db: Session) -> None:
    run = seed_run(db)
    run_validate = remove_run(db, run["obj"].id)
